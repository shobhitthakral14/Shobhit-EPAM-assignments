// shared pointer is a smart pointer who manages the lifetime of dynamically created object through reference counting 

//A shared_ptr<T> holds a shared ownership of a dynamically allocated object of type T.
// multiple shared_ptr can point to single object 
// the object destroys automatically when the last shared_ptr woning it is destroyed or reset 
// Internally, it uses a reference count to track how many shared_ptrs share ownership of the object.
...
//When a new shared_ptr is created from an existing one (copy constructor or assignment), the reference count increases.

// When a shared_ptr is destroyed or reset, the reference count decreases.

// When reference count reaches zero, the managed object is deleted.
...
// When multiple parts of the program need access to the same object, and you want automatic memory management. IT IS USED 
class Test {
    public:
...
// PASSING SHARED POINTER TO THE FUNCTION
    ...
    /// Never pass the same raw pointer to multiple shared_ptrs — it causes double deletion.
    // int* x = new int(5);
    //shared_ptr<int> sp1(x);
    //shared_ptr<int> sp2(x); // ❌ Undefined behavior
